<?php
// facturas_pagos.php (con filtros por sección + recordar acordeón/posición al volver)
...
// recordar qué acordeón y qué fila/elemento enfocar al volver
$openFromReturn  = trim($_GET['open'] ?? '');   // 'empresa' | 'clientes'
$focusFromReturn = trim($_GET['focus'] ?? '');  // id del elemento (tr o contenedor)
...
$emp_desde  = trim($_GET['emp_desde'] ?? ''); // YYYY-MM-DD
$emp_hasta  = trim($_GET['emp_hasta'] ?? ''); // YYYY-MM-DD
...
$cli_cliente = trim($_GET['cli_cliente'] ?? ''); // nombre o email cliente
...
<head>
    <meta charset="UTF-8">
    <title>Gestión de pagos - EcoHome</title>
    <link rel="stylesheet" href="../../../public/assets/css/style.css">
</head>
<body>

<header class="header">
    <h1>Facturas sin pagar</h1>
    <p>
...
<section class="dashboard-box" style="max-width:1200px; text-align:left;">
    ...
    <?php if ($ok === 'empresa_pagada'): ?>
        <p class="success">Factura de empresa marcada como pagada ✅</p>
    <?php elseif ($ok === 'cliente_pagada'): ?>
        <p class="success">Factura de cliente marcada como pagada ✅</p>
    <?php endif; ?>
    ...
    <!-- Acordeón: Empresa -->
    <!-- ========================= -->
    <details class="acordeon" id="acc_empresa" <?= $openEmpresaAcordeon ? 'open' : '' ?>>
        <summary class="acordeon__summary">
            <span>Facturas de la empresa · Pendientes</span>
        ...
        <div class="acordeon__content">
            ...
            <!-- Filtro interno -->
            <details class="acordeon" id="filtro_empresa" style="margin:0 0 18px 0;" <?= $openEmpresaFiltro ? 'open' : '' ?>>
                <summary class="acordeon__summary">
                ...
                <div class="acordeon__content">
                    ...
                    <form method="GET" class="form-grid" style="max-width:950px;">
                        ...
                        <a class="link-btn" href="facturas_pagos.php" style="text-align:center;">Limpiar</a>
                    </form>
                    ...
                    <table class="tabla">
                        ...
                        <?php foreach ($factEmpresa as $f): ?>
                            ...
                            <tr id="<?= htmlspecialchars($rowId) ?>">
                                ...
                                <td>
                                    <a
                                      href="factura_empresa_pagar.php?id=<?= (int)$f['id'] ?>&back=facturas_pagos&open=empresa&focus=acc_empresa"
                                ...
                                <td>
                                    <a href="../pedidos/pedido_ver.php?id=<?= (int)$f['pedido_id'] ?>&back=facturas_pagos&open=empresa&focus=<?= htmlspecialchars($rowId) ?>">
                                        Ver pedido
                                    </a>
                                </td>
                            </tr>
    ...
    <!-- Acordeón: Clientes -->
    <!-- ========================= -->
    <details class="acordeon" id="acc_clientes" <?= $openClientesAcordeon ? 'open' : '' ?>>
        <summary class="acordeon__summary">
            <span>Facturas a clientes · Pendientes</span>
        ...
        <div class="acordeon__content">
            ...
            <!-- Filtro interno -->
            <details class="acordeon" id="filtro_clientes" style="margin:0 0 18px 0;" <?= $openClientesFiltro ? 'open' : '' ?>>
                <summary class="acordeon__summary">
                    ...
                    <table class="tabla">
                        ...
                        <?php foreach ($factClientes as $fc): ?>
                            ...
                            <tr id="<?= htmlspecialchars($rowId) ?>">
                                ...
                                <td>
                                    <a
                                      href="factura_cliente_pagar.php?id=<?= (int)$fc['id'] ?>&back=facturas_pagos&open=clientes&focus=acc_clientes"
                                      onclick="return confirm('¿Marcar esta factura de cliente como pagada?');"
                                    >
                                        Marcar pagada
                                    </a>
                                </td>
                                <td>
                                    <?php if (!empty($fc['archivo_path'])): ?>
                                        <a href="ver_factura_cliente.php?id=<?= (int)$fc['id'] ?>&back=facturas_pagos&open=clientes&focus=<?= htmlspecialchars($rowId) ?>">
                                            Ver imagen
                                        </a>
                                    <?php else: ?>
                                        —
...
<footer class="footer">
    <p>© 2026 EcoHome · Gestión de pagos</p>
</footer>
...
(function () {
    ...
    // Si hay focus, intentamos ir ahí; si no existe (porque desapareció), caemos al acordeón.
    const goTo = (id) => {
    ...
    if (focus) {
        if (goTo(focus)) return;
    }

    if (open === 'empresa') goTo('acc_empresa');
    if (open === 'clientes') goTo('acc_clientes');
})();
</script>
